...
if(!$gid && (!defined('FORUM_INDEX_PAGE_MEMORY') || !FORUM_INDEX_PAGE_MEMORY)) {
	...
	$forums = C::t('portal_tx_ffc')->order_by_id(1);
	//debug($forums);
	$ids = array();
	foreach($forums as $forum) {
		$ids[$forum['id']] = $forum['id'];
	}
	
	$forums = C::t('portal_cqssc')->fetch_all(1);
	//debug($forums);
	$datas = array();
	$index = 0;
	//$arr = array_column($forums, 'id');
	foreach($forums as $forum => $value) {
		foreach ($value as $k => $v) {
			$datas[$index][$k]=$v;
		}
		//$datas[$forum['id']] = $forum;
		$index ++;
	}
	...
	/*$forums = C::t('forum_forum')->fetch_all_by_status(1);//debug($forums);
	$fids = array();
	foreach($forums as $forum) {
