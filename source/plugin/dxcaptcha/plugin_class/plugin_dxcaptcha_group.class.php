...
* 
*/
class plugin_dxcaptcha_group extends plugin_dxcaptcha
{
	...
	function post_middle(){
        if ($this->_cur_mod_is_valid()) {
            ...
            return $this->_code_output($cur_mod ,$dx_captcha_id, $page_type, "dx_verify_reply_grade") . $this->_fix_post_middle();
        }
    }
...
JS;
    ...
    function post_infloat_middle() { 
        if ($this->_cur_mod_is_valid()) {
            ...
            return $this->_code_output($cur_mod, $dx_captcha_id, $page_type, "dx_verify_reply_float");
        }
    }
    //页面底部发表回复
    ...
    function forumdisplay_postbutton_top(){
        if ($this->_cur_mod_is_valid()) {
            ...
            return $this->_code_output($cur_mod, $dx_captcha_id, "", $btn_id).$this->_fix_zhibo_reply($dx_captcha_id);
        }
    ...
    <script type="text/javascript">
        function move_fast_dxcaptha_before_submit() {
            if($('livereplysubmit')){
                ...
                livereplysubmit.parentNode.appendChild(input);
                
                //livereplysubmit.parentNode.insertBefore(dxcaptha, livereplysubmit);

                livereplysubmit.parentNode.appendChild(captcha_id);
            }else{
...
JS;
    ...
    <script type="text/javascript">
        function move_dxcaptha_before_submit() {
            ...
            var dxcaptha = document.getElementById('$dx_captcha_id');
            gt_submitBtn.parentNode.appendChild(dxcaptha);
        }
...
JS;
    ...
    function forumdisplay_fastpost_btn_extra() {
        if ($this->_cur_mod_is_valid()) {
            ...
            return $this->_code_output($cur_mod, $dx_captcha_id, $page_type, "dx_verify_forumdisplay").$this->_fix_fastpost_btn_extra_pos($dx_captcha_id);
        }
    }
