 ...
 * $Id: forum_attachment.php 13222 2018-03-23 14:34:49Z zhuge $
...
define('NOROBOT', TRUE);
@list($_GET['aid'], $_GET['k'], $_GET['t'], $_GET['uid'], $_GET['tableid']) = daddslashes(explode('|', base64_decode($_GET['aid'])));/*1.3.1.4.学.习.网*/
$aid = intval($_GET['aid']);
if(!empty($_GET['findpost']) && ($attach = C::t('forum_attachment')->fetch($aid))) {
dheader('location: forum.php?mod=redirect&goto=findpost&pid='.$attach['pid'].'&ptid='.$attach['tid']);/*29043*/
}
if($_GET['uid'] != $_G['uid'] && $_GET['uid']) {
$_G['uid'] = $_GET['uid'] = intval($_GET['uid']);//1.3.1.4.学.习.网
$member = getuserbyuid($_GET['uid']);
loadcache('usergroup_'.$member['groupid']);
$_G['group'] = $_G['cache']['usergroup_'.$member['groupid']];
$_G['group']['grouptitle'] = $_G['cache']['usergroup_'.$_G['groupid']]['grouptitle'];#版权：1314学习网，未经允许不得公开出售、发布、使用、修改，如需购买请联系我们获得授权
...
$aid = intval($aid);/*www_discuz_1314study_com*/
if($attach = C::t('forum_attachment_n')->fetch($tableid, $aid)) {
if($attach['isimage']) {
dheader('location: '.$_G['siteurl'].'static/image/common/none.gif');
} else {
if(!$requestmode) {
showmessage('attachment_expired', '', array('aid' => aidencode($aid, 0, $attach['tid']), 'pid' => $attach['pid'], 'tid' => $attach['tid']));#1314学习网
} else {
exit;
}
}
...
periodscheck('attachbanperiods');//1314学习网
...
$attach = C::t('forum_attachment_n')->fetch($tableid, $aid);#12415
$thread = C::t('forum_thread')->fetch_by_tid_displayorder($attach['tid'], 0, '>=', null, $archiveid);
if($_G['uid'] && $attach['uid'] != $_G['uid']) {
if($attach) {
$attachpost = C::t('forum_post')->fetch($thread['posttableid'], $attach['pid'], false);//版权：www.1314study.com
...
if(!$requestmode) {
$forum = C::t('forum_forumfield')->fetch_info_for_attach($thread['fid'], $_G['uid']);#1.3.1.4.学.习.网
$_GET['fid'] = $forum['fid'];
if($attach['isimage']) {
$allowgetattach = !empty($forum['allowgetimage']) || (($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid']) && !$forum['getattachperm']) || forumperm($forum['getattachperm']);/*www_discuz_1314study_com*/
} else {
...
showmessage('attachment_forum_nopermission', NULL, array(), array('login' => 1));/*www_discuz_1314study_com*/
}
...
$exemptvalue = $ismoderator ? 64 : 8;//版权：www.1314study.com
...
$db = DB::object();//5467
$db->close();/*From www.1314study.com*/
...
if($attach['remote']) {
$_G['setting']['ftp']['hideurl'] ? getremotefile(getimgthumbname($attach['attachment'])) : dheader('location:'.$_G['setting']['ftp']['attachurl'].'forum/'.getimgthumbname($attach['attachment']));/*版权：1314学习网，未经允许不得公开出售、发布、使用、修改，如需购买请联系我们获得授权*/
} else {
...
if(in_array($thread['fid'], $study_fids)){
	if(!$attach['isimage'] || $splugin_setting['study_imageradio']){
			$attach['filename'] = str_replace($splugin_setting['study_prefix'], '', $attach['filename']);
			$attach['filename'] = $splugin_setting['study_prefix'].$attach['filename'];
			$study_time = intval($splugin_setting['study_time']) ? intval($splugin_setting['study_time']) : 60;
			if($uid){
				$log = C::t('#study_diydown#study_diydown_log')->fetch_first_by_aid_uid($aid, $_G['uid']);
			}else{
				$log = C::t('#study_diydown#study_diydown_log')->fetch_first_by_aid_ip($aid, $_G['clientip']);		
			}
			if($_G['timestamp'] - $log['dateline'] > $study_time){
				$data = array('aid' => $aid, 'uid'=> intval($_G['uid']), 'ip'=> $_G['clientip'], 'dateline'=> $_G['timestamp']);
				C::t('#study_diydown#study_diydown_log')->insert($data);
			}
	}
}
//@dly END
...
//Copyright 2001-2099 1314学习网.
//This is NOT a freeware, use is subject to license terms
//$Id: forum_attachment.php 13675 2018-03-23 06:34:49Z zhuge $
//应用售后问题：http://www.1314study.com/services.php?mod=issue
//应用售前咨询：QQ 15326940
//应用定制开发：QQ 643306797
//本插件为 1314学习网（www.1314study.com） 独立开发的原创插件, 依法拥有版权。
