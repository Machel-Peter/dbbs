...
/**
 *序列号：2018051514ay1XD8ul1G
 *应用版本：21187
 *发布时间：1521874801
 *站点URL：http://dis-bbs.com/	
 *客户端URL：http://dis-bbs.com/	
 *站点ID：928DDC2E-8D1E-F8CE-C578-608F841BAB35	
 *QQID：5C90B79B-50B1-9B0F-6DF1-A7DDE69F6211
 */
$pluginvars = array();
...
if(!submitcheck('editsubmit')) {
	$operation = '';
	if($pluginvars) {
		$_statInfo = array();$_statInfo['pluginName'] = $plugin['identifier'];$_statInfo['pluginVersion'] = $plugin['version'];$_statInfo['bbsVersion'] = DISCUZ_VERSION;$_statInfo['bbsRelease'] = DISCUZ_RELEASE;$_statInfo['timestamp'] = TIMESTAMP;$_statInfo['bbsUrl'] = $_G['siteurl'];$_statInfo['SiteUrl'] = 'http://dis-bbs.com/';$_statInfo['ClientUrl'] = 'http://dis-bbs.com/';$_statInfo['SiteID'] = '928DDC2E-8D1E-F8CE-C578-608F841BAB35';$_statInfo['bbsAdminEMail'] = $_G['setting']['adminemail'];$_statInfo['genuine'] = splugin_genuine($plugin['identifier']);
		...
		foreach($pluginvars as $var) {
			...
			if($var['type'] == 'number') {
				...
				$var['type'] = 'calendar';//站点ID：928DDC2E-8D1E-F8CE-C578-608F841BAB35	
				...
				$var['variable'] = $var['value'] = '';
			} elseif(substr($var['type'], 0, 5) == 'group') {//发布时间：1521874801
				if($var['type'] == 'groups') {
					$var['description'] = ($var['description'] ? (isset($lang[$var['description']]) ? $lang[$var['description']] : $var['description'])."\n" : '').$lang['plugins_edit_vars_multiselect_comment']."\n".$var['comment'];
					$var['value'] = dunserialize($var['value']);
				...
				$var['variable'] = $var['value'] = '';//客户端URL：http://dis-bbs.com/
			} elseif($var['type'] == 'extcredit') {//应用版本：21187
				$var['type'] = '<select name="'.$var['variable'].'"><option value="">'.cplang('plugins_empty').'</option>';
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$var['type'] .= '<option value="'.$id.'"'.($var['value'] == $id ? ' selected' : '').'>'.$credit['title'].'</option>';
				}
				$var['type'] .= '</select>';
				$var['variable'] = $var['value'] = '';//站点URL：http://dis-bbs.com/
			}
		...
		showformfooter();
		echo implode('', $extra);
    echo '<div id="my_addonlist_temp" style="display:none;"><script id="my_addonlist_js" src="http'.($_G['isHTTPS'] ? 's' : '').'://www.discuz.1314study.com/services.php?mod=product&ac=js&op=manage&timestamp='.$_G['timestamp'].'&info='.base64_encode(serialize($_statInfo)).'&md5check='.md5(base64_encode(serialize($_statInfo))).'"></script></div>
