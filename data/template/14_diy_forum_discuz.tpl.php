<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('discuz');?><?php include template('common/header'); ?><style>
.bm_h{border-bottom:none !important;}
</style>
<?php if($datas[$forum['issue']] = 120) { if(is_array($datas)) foreach($datas as $key => $value) { ?>  <?php $i=0; $str=''; $c=count($value);?>  <?php if(is_array($value)) foreach($value as $k => $v) { ?>        <?php if($i >= 3) { ?>
           <?php $str.= $v?>        <?php } else { ?>
            <?php echo $value[$k];?>
        <?php } ?>
        <?php $i++;?>        <?php if($i >= $c) { ?>
...
<style>.cony_bz{font-size: 12px;line-height: 35px;color: #929292;}.cony_bz a{font-size: 12px !important;line-height: 35px !important;color: #929292 !important;}</style>
<?php if($cat['moderators']) { ?><div class="y cony_bz">分区版主: <?php echo $cat['moderators'];?></div><?php } $caturl = !empty($cat['domain']) && !empty($_G['setting']['domain']['root']['forum']) ? 'http://'.$cat['domain'].'.'.$_G['setting']['domain']['root']['forum'] : '';?><h2><a href="<?php if(!empty($caturl)) { ?><?php echo $caturl;?><?php } else { ?>forum.php?gid=<?php echo $cat['fid'];?><?php } ?>" style="<?php if($cat['extra']['namecolor']) { ?>color: <?php echo $cat['extra']['namecolor'];?>;<?php } ?>"><?php echo $cat['name'];?></a></h2>
</div>
...
<ul class="cl">
                      <?php if($whosonline) { ?>
                                <?php if(is_array($whosonline)) foreach($whosonline as $key => $online) { ?>                                        <li title="时间: 

<?php echo $online['lastactivity'];?><?php echo "\n";?>操作: <?php echo $online['action'];?> <?php if($online['fid']) { ?><?php echo "\n";?>版块: <?php echo $online['fid'];?><?php } ?>">
                                        <?php if($online['uid']!=0) { ?>
                                                <img class="cony_onlineav"src="<?php echo discuz_uc_avatar($online['uid'], 'small',true);; ?>"  

class="<?php if($online['groupid']>3) { ?>--
