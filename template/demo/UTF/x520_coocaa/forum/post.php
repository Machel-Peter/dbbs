...
<div id="pt" class="bm cl">
	<div class="z"><a href="./" class="nvhm" title="{lang homepage}">首页</a> <em>&rsaquo;</em> <a href="forum.php?mod=forumdisplay&fid=$_G['forum'][fid]">$_G['forum'][name]</a> <em>&rsaquo;</em> $actiontitle</div>
</div>
...
<form method="post" autocomplete="off" id="postform"
	{if $_GET[action] == 'newthread'}action="forum.php?mod=post&action={if $special != 2}newthread{else}newtrade{/if}&fid=$_G[fid]&extra=$extra&topicsubmit=yes" $enctype
...
<div id="ct" class="ct2_a ct2_a_r wp cl">
	<input type="hidden" name="coverfid" id="coverfid" value="{$_GET['fid']}" />
	...
	<!--{/if}-->
	<div class="bm bw0 cl ccedit yanjiao"{if !$showthreadsorts && !$adveditor} id="editorbox"{/if}>
		<!--{if $_GET[action] == 'newthread'}-->
			<ul class="tb cl">
				<!--{if $savecount}-->
		...
		<div id="postbox">
            ...
            <dl class="posot_col cl editbox">
              <dt class="ed"> 
               <span>* </span>我的正文：
              </dt>
			  <dd class="ec"> <!--{subtemplate forum/post_editor_body}--></dd>
            </dl>
			<!--{hook/post_middle}-->
			...
			<!--{if $_GET[action] != 'edit'}-->
				<div id="seccheck" class="ptm cl post_tag">
            ...
            <!--{if $_G['group']['allowposttag']}-->
				<div id="extra_tag_c" class="cl" >
					<table cellspacing="0" cellpadding="0">
						<tr>
							<td class="xw1"><span>&nbsp; </span>添加{lang post_tag}：</td>
							<td>
						...
						<tr>
							<td></td>
							<td>
								<p class="xg1" style="padding-top:5px;">准确的标签有助于让更多跟你共同爱好的人第一时间看到你的文章哟！用逗号或空格隔开多个标签，最多可填写5个</p>
								<!--{eval $recent_use_tag = recent_use_tag();}-->
								<!--{if $recent_use_tag}-->
									<p class="mtn"><b>想不到标签？ 这里有些热门标签</b>
                                        
										<!--{eval $tagi = 0;}-->
										<!--{loop $recent_use_tag $var}-->
											<!--{if $tagi}--><span>|</span><!--{/if}--><a href="javascript:;" class="xi2" onclick="$('tags').value == '' ? $('tags').value += '$var' : $('tags').value += ',$var';extraCheck(4);doane();">$var</a>
											<!--{eval $tagi++;}-->
										<!--{/loop}-->
			...
			<div class="mtm mbm pnpost">
				...
				<button type="submit" id="postsubmit" class="pn sb yanjiao" value="true" name="{if $_GET[action] == 'newthread'}topicsubmit{elseif $_GET[action] == 'reply'}replysubmit{elseif $_GET[action] == 'edit'}editsubmit{/if}">
				<!--{if $_GET[action] == 'newthread'}-->
				...
				<!--{if $_G['uid']}-->
					...
					<!--{if $_GET[action] == 'newthread' && !$modnewthreads || $_GET[action] == 'edit' && $isfirstpost && !$modnewreplies && $thread['displayorder'] == -4}-->
						<button type="button" id="postsubmit" class="pn yanjiao" value="true" onclick="$('postsave').value = 1;$('postsubmit').click();"><span>{lang save_draft}</span></button>
					<!--{/if}-->
				<!--{/if}-->
				<!--{hook/post_btn_extra}-->
