...
<div id="{if $_G['forum']['allowside']}ctl{else}ctls{/if}" class="pl" style="margin-top:20px;">
    <div class="cttop nbk">
        <div class="main_cat"><a href="forum.php?mod=forumdisplay&fid=$_G['forum'][fid]">$_G['forum'][name]</a></div>
        <!--{if $_GET['page']==1 || $_GET['page']=='' || $_GET['page']>1}-->
        <div class="post_title">
            <h2>
                <span id="thread_subject">$_G[forum_thread][subject]</span>
                <span class="xg1" style="font-size:12px">
                    <a href="forum.php?mod=viewthread&amp;tid={$_G[forum_thread][tid]}&amp;fromuid={$_G[forum_thread][tid]}" onclick="return copyThreadUrl(this, 'bbs.coocaa.com')" title="您的朋友访问此链接后，您将获得相应的积分奖励">[复制链接]</a>
                </span>
                <div class="clear"></div>
            </h2>
            <div class="nei_left2_tit">
                <ul>
                    <li>
                        <a href="forum.php?mod=forumdisplay&fid=$_G[fid]">[<!--{$_G['forum'][name]}-->]</a>
                          <span><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" title="$_G[forum_thread][author]" class="author">$_G[forum_thread][author]</a><!--{eval echo date('Y-m-d H:i',$_G[forum_thread][dateline])}--> 
                    ...
                    {if $_G[forum_thread][displayorder]}<li><span>置顶</span></li>{/if}
                    {if $_G[forum_thread][digest]}<li><span>精华</span></li>{/if}
                </ul>
           
            <div class="post_title_r">
                <dl>
                    <dd class="view" title="查看次数"><span>$_G[forum_thread][views]</span></dd>
                    {if $_G['cahce']['bi_digg']['number']}
                    <dd class="like"><a href="javascript:;">{$_G['cahce']['bi_digg']['number']}</a></dd>
                    {/if}
                    <dd class="comment" title="评论数"><span><a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$post[pid]&extra=$_GET[extra]&page=$page"  onclick="showWindow('reply', this.href)">{$all_replies}</a></span></dd>
                    ...
                    <dt class="fx_lj">
                      <div class="bdsharebuttonbox" style=" margin-top:-7px; "><a href="#" class="bds_more" data-cmd="more"></a><a title="分享到QQ空间" href="#" class="bds_qzone" data-cmd="qzone"></a><a title="分享到微博" href="#" class="bds_tsina" data-cmd="tsina"></a><a title="分享到微信" href="#" class="bds_weixin" data-cmd="weixin"></a></div>
    ...
    <div id="ct" class="wp cl" style="border-top:none;">
    ...
    <div id="postlist" class="pl veiwlist"> 
    ...
    <div id="pl_top">
        <table cellspacing="0" cellpadding="0">
          ...
          <!--{if $_G['forum_thread']['replycredit'] > 0 }-->
            ...
            <!--{/if}--> 
                <img src="{IMGDIR}/thread_prize_s.png" class="hm" alt="{lang replycredit}" /> <strong>{$_G['forum_thread']['replycredit']} {$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][unit]}{$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][title]}</strong> 
                <!--{if !$close_leftinfo}-->
                </td>
                ...
                {lang thread_replycredit_tips1} {lang thread_replycredit_tips2}<!--{if $_G['forum_thread']['replycredit_rule'][random] > 0}--><span class="xg1">{lang thread_replycredit_tips3}</span><!--{/if}--></td>
            </tr>
          ...
          <tr> 
            ...
            <td class="pls vm ptm"><img src="{IMGDIR}/rushreply_s.png" class="vm" alt="{lang rushreply}" /> <strong>{lang rushreply}</strong></td>
            <td class="plc ptm pbm xi1"><!--{else}-->
              
            <td class="plc ptm pbm xi1" colspan="2"><img src="{IMGDIR}/rushreply_s.png" class="vm" alt="{lang rushreply}" /> 
              ...
              <p class="ptn"> 
                ...
                <!--{if $countrushpost}-->[<strong>$countrushpost</strong>]{lang thread_rushreply_rewardnum}<!--{else}--> {lang thread_rushreply_noreward} <!--{/if}-->&nbsp;&nbsp; <a href="forum.php?mod=viewthread&tid=$_G[tid]" class="xi2">{lang thread_rushreply_check_back}</a> </p>
      ...
      <!--{/loop}-->
        ...
        <div class="pgs mbm nbk cl vt" id="viewpage"> 
            <!--{template forum/viewthread_page}--> 
        </div>
  ...
  $_G['forum_tagscript'] </div>
<!--{if $_G['forum']['allowside']}-->
<div class="box_right" style="margin-top:20px;">
  <div id="ctr">
    <div class="nei_rig1 nbk">
      	<div class="nei_rig1_c">
			<!--{eval $postlist = array_slice($postlist,0,1)}-->
			<!--{loop $postlist $post}-->
	        <div class="cl">
		        <p>
		        	<a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" class="avtm" target="_blank"><!--{eval $svipz = DB::fetch_first("SELECT * FROM ".DB::table('common_member_verify')." WHERE uid=$post[authorid] ");}--><!--{if $svipz[verify6] ==1}--><span class="addvip"></span><!--{/if}--><!--{if $_G[forum_thread][authorid] && $_G[forum_thread][author]}--><!--{avatar($_G[forum_thread][authorid],big)}--><!--{else}--><img src="$_G['style']['directory']/images/anonymity_1.jpg" /><!--{/if}--></a>
		       	</p>
		        <b>
			        <h2>
			        	<span>
		        		<!--{if $_G[forum_thread][authorid] && $_G[forum_thread][author]}-->
		        		<a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" target="_blank" class="xw1"{if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$_G[forum_thread][author]</a>
		        		<!--{else}-->
			        	{lang anonymous}
			        	<!--{/if}-->
			        	</span>
			        ...
			        <ul>
			          <li><a class="xi2" onclick="showWindow(this.id, this.href, 'get', 0);" id="a_friend_li_{$_G[forum_thread][authorid]}" title="加为好友" href="home.php?mod=spacecp&ac=friend&op=add&uid={$_G[forum_thread][authorid]}&handlekey=addfriendhk_{$_G[forum_thread][authorid]}"><img src="$_G['style']['directory']/images/nei_37.jpg" border="0" alt="加为好友" /></a></li>
			          <li><a id="a_sendpm_{$_G[forum_thread][authorid]}" title="发送消息" onclick="showWindow('showMsgBox', this.href, 'get', 0)" href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_{$_G[forum_thread][authorid]}&touid={$_G[forum_thread][authorid]}&pmid=0&daterange=2"><img src="$_G['style']['directory']/images/nei_39.jpg" border="0"  alt="发送消息"/></a></li>
			        </ul>
			        <div class="clear"></div>
			        <!--{eval //print_r($grouptitle)}-->
			        <strong class="cl">
			        	<!--{if $_G[forum_thread][authorid] && $_G[forum_thread][author]}-->
			        		<!--{if $post['groupid'] ==1}-->
			        		<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="mlv yanjiao" title="等级">管理员</a>
			        		<!--{elseif $post['groupid'] ==2}-->
			        		<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="mlv yanjiao" title="等级">超级版主</a>
			        		<!--{elseif $post['groupid'] ==3}-->
			        		<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="mlv yanjiao" title="等级">版主</a>
			        		<!--{else}-->
			        		<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="mlv yanjiao" title="等级">LV<!--{$post['stars']}--></a>
			        		<!--{/if}-->
			        		<a href="javascript:;" class="mscore yanjiao" title="{$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]][title]}">{$post[extcredits2]}</a>
			        	<!--{else}-->
			        	<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="mlv yanjiao" title="等级">LV0</a><a href="javascript:;" class="mscore yanjiao" title="{$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]][title]}">0</a>
			        	<!--{/if}-->
			        </strong>
		        </b>
	        </div>
	        <div class="md_ctrl md_ctrl_u" id="r_medal" data-count="7">
	        	<a href="home.php?mod=medal" class="md_link">
			...
			<div class="md_show">
			  	<a class="md_show_down" title=""></a>
			  	<a class="md_show_up" title="收起勋章"></a>
			</div>
			<!--{/loop}-->
			<script type="text/javascript">
			 jQuery(function(){
				 ...
				 $(".md_ctrl_u").each(function (){
					 var num = $(this).find(".md_link:first").find("img").size();
					 $(this).next().find('.md_show_down').attr('title','展开勋章，总共'+num+'枚');
					 var count = $(this).attr("data-count") || 5;
					 if(count>=num){
						 $(this).next().hide();
					 }else 
				 ...
				 jQuery('.md_show_down').click(function(){
				   jQuery(this).parent().prev('.md_ctrl_u').css('height','auto');
				   jQuery(this).hide().siblings().css("display", "block");
				 });
				 jQuery(".md_show_up").click(function(){
					 jQuery(this).parent().prev(".md_ctrl_u").css("height", "25px");
					 jQuery(this).hide().siblings().css("display", "block");
					 });
			 })
		    </script>
    ...
    <!--[diy=diyvt01]--><div id="diyvt01" class="area"></div><!--[/diy]-->

    <div id="ct">
        <div class="sd nbk">
            <!--[diy=diyvt02]--><div id="diyvt02" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt03]--><div id="diyvt03" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt04]--><div id="diyvt04" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt05]--><div id="diyvt05" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt06]--><div id="diyvt06" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt07]--><div id="diyvt07" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt08]--><div id="diyvt08" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt09]--><div id="diyvt09" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt10]--><div id="diyvt10" class="area"></div><!--[/diy]-->

            <!--[diy=diyvt11]--><div id="diyvt11" class="area"></div><!--[/diy]-->
        </div>
		</div>
    </div>
</div>
...
<div class="viewpost vpp">
    <div class="vpshare cl">
    	<!--{if !$_G['forum_thread']['is_archived']}--><a id="newspecial" style="float:none;"  class="vpa" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})" href="forum.php?mod=post&action=newthread&fid=$_G[fid]" title="{lang send_posts}" ><i></i>发表主题</a><!--{/if}-->
    	<a href="javascript:bi_digg();" class="like" style="display: none;"><i></i>点赞</a> <a href="<!--{eval echo $_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']}-->#f_pst" class="replay" target="_self"><i></i>回复</a>
        <a href="#" class="bds_more bdsharebuttonbox share" data-cmd="more" ><i></i>分享</a>
