...
<!--{else}-->
	...
	<!--{if !$post['first'] && $hiddenreplies && $_G['forum']['ismoderator']}-->
		<div class="locked" style="display:none">{lang message_ishidden_hiddenreplies}</div>
	<!--{/if}-->
	<!--{if $post['first']}-->
	...
	<div class="{if !$_G[forum_thread][special]}t_fsz{else}pcbs{/if}">
		...
		<!--{if $post['first']}-->
			<!--{if !$_G[forum_thread][special]}-->
				...
				<!--{eval echo str_replace('http://t.cn','',str_replace('http://dwz.cn','',$post['message']));}-->
				</td></tr></table>
		...
		<!--{else}-->
		<!--{eval //out($post)}-->
		<!--{if !$post['first'] && $hiddenreplies && !$_G['forum']['ismoderator'] && !($_G[forum_thread]['authorid']==$_G['uid']) && !($post['authorid']==$_G['uid'])}-->
			<div class="locked">{lang message_ishidden_hiddenreplies}</div>
		<!--{else}-->			
			<table cellspacing="0" cellpadding="0"><tr><td class="t_f" id="postmessage_$post[pid]">   
			...
			<!--过滤t.cn 和dwz.cn-->
			<!--{eval echo str_replace('http://t.cn','',str_replace('http://dwz.cn','',$post['message']));}-->
			<!--{else}--><span class="xg1">{lang moderate_need}</span><!--{/if}--></td></tr>
			</table>
		<!--{/if}-->
        ...
        <!--{if (($_G['forum']['ismoderator'] && $_G['group']['alloweditpost'] && (!in_array($post['adminid'], array(1, 2, 3)) || $_G['adminid'] <= $post['adminid'])) || ($_G['forum']['alloweditpost'] && $_G['uid'] && ($post['authorid'] == $_G['uid'] && $_G['forum_thread']['closed'] == 0) && !(!$alloweditpost_status && $edittimelimit && TIMESTAMP - $post['dbdateline'] > $edittimelimit)))}-->
			...
			<!--{if $post['invisible'] == 0}-->
	            <!--{if !IS_ROBOT && !$_GET['authorid'] && !$_G['forum_thread']['archiveid']}-->
	            	&nbsp;&nbsp;&nbsp;
	                <a href="forum.php?mod=viewthread&tid=$post[tid]&page=$page&authorid=$post[authorid]" rel="nofollow">只看该作者</a>
			...
			<!--{if (!$_G['uid'] || $allowpostreply) && !$needhiddenreply}-->
				<!--{if $post['first']}-->
					<a class="cmmnt2 y" href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$post[pid]&extra=$_GET[extra]&page=$page" onclick="showWindow('reply', this.href)">{lang reply}</a>
				<!--{else}-->
					<a class="cmmnt2 y" href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&repquote=$post[pid]&extra=$_GET[extra]&page=$page" onclick="showWindow('reply', this.href)">{lang reply}</a>
				<!--{/if}-->
			<!--{/if}-->
		<!--{/if}-->
   	<!--{/if}-->
...
<div class="clear"></div>
	...
	<div id="comment_$post[pid]" class="cm reply">
		<!--{if $totalcomment[$post[pid]]}--><div class="pstl">$totalcomment[$post[pid]]</div><!--{/if}-->
        {eval $res_comm = array_reverse($comments[$post[pid]])}
		<!--{loop $res_comm $comment}-->
                    ...
                    <div class="psti_h">
                      ...
                      <!--{if $comment['authorid']}-->
						<a href="home.php?mod=space&uid=$comment[authorid]" class="xi2 xw1">$comment[author]</a>
                             回复
                        	{if $comment['p_field']}
                            {eval $_r = unserialize($comment['p_field'])}
                            <a href="home.php?mod=space&uid={$_r[authorid]}" class="xi2 xw1">{$_r[author]}</a>
                            {else}
                            <a href="home.php?mod=space&uid=$post['authorid']" target="_blank" class="xi2 xw1">$post[author]</a> 
                            {/if} 
						<!--{else}-->
						{lang guest}
					<!--{/if}-->
                      ...
                      <span class="xg1">
					    {eval echo date('Y-m-d H:i:s',$comment[dateline])}
						
					  </span>
					  <!--{if ($post['authorid'] != $_G['uid'] && $_G['uid'] != 1)}-->
						<a class="report"  href="javascript:;" onclick="showWindow('miscreport$post[pid]', 'misc.php?mod=report&rtype=post&rid=$post[pid]&tid=$_G[tid]&fid=$_G[fid]', 'get', -1);return false;">{lang report}</a>
					<!--{/if}-->
                      <!--{if $allowpostreply && $post['allowcomment'] && (!$thread['closed'] || $_G['forum']['ismoderator'])}--><a class="cmmnt2" href="forum.php?mod=misc&action=comment&tid=$post[tid]&pid=$post[pid]&commentid={$comment[id]}&extra=$_GET[extra]&page=$page{if $_G['forum_thread']['special'] == 127}&special=$specialextra{/if}" onclick="showWindow('comment', this.href, 'get', 0)">{lang reply}</a><!--{/if}-->
                    </div>
<!--{eval //out($comment)}-->
                    <!--{if !$post['first'] && $hiddenreplies && !$_G['forum']['ismoderator'] && !($_G[forum_thread]['authorid']==$_G['uid']) && !($comment['authorid']==$_G['uid'])}-->
					<div class="locked">{lang message_ishidden_hiddenreplies}</div>
					<!--{else}-->	
					$comment[comment]&nbsp;
					<!--{/if}-->
	...
	<!--{else}-->
	<div id="comment_$post[pid]" class="cm reply" style="display:none">
