...
<div id="scbar" class="{if $_G['setting']['srchhotkeywords'] && count($_G['setting']['srchhotkeywords']) > 5}scbar_narrow {/if}cl">
	<form id="scbar_form" class="site-search-form" method="{if $_G[fid] && !empty($searchparams[url])}get{else}post{/if}" autocomplete="off" onsubmit="searchFocus($('scbar_txt'))" action="{if $_G[fid] && !empty($searchparams[url])}$searchparams[url]{else}search.php?searchsubmit=yes{/if}" target="_blank">
        ...
        <input type="text" class="search-input xg1 xg1" name="srchtxt" id="scbar_txt" value="{lang enter_content}" autocomplete="off" x-webkit-speech="" speech="">
        <span class="wdao_scbar_type"><a href="javascript:;" id="scbar_type" class="xg1 showmenu" onclick="showMenu(this.id)" hidefocus="true">{lang search}</a></span>
        </div>
        <button type="submit" name="searchsubmit" id="scbar_btn" sc="1" class="search-btn button-primary" value="true">搜索</button>		

	</form>
</div>
<ul id="scbar_type_menu" class="p_pop wdao_scbar_type_menu" style="display: none;"><!--{echo implode('', $slist);}--></ul>
